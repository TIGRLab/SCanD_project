...
for subj_id in `ls -d ${SUBJECTS_DIR}/${PREFIX}*`; do #may need to change this so that it selects subjects with FS output
  ...
  for side in lh.aparc.stats rh.aparc.stats; do
    ...
    for x in bankssts caudalanteriorcingulate caudalmiddlefrontal cuneus entorhinal fusiform inferiorparietal inferiortemporal isthmuscingulate lateraloccipital lateralorbitofrontal lingual medialorbitofrontal middletemporal parahippocampal paracentral parsopercularis parsorbitalis parstriangularis pericalcarine postcentral posteriorcingulate precentral precuneus rostralanteriorcingulate rostralmiddlefrontal superiorfrontal superiorparietal superiortemporal supramarginal frontalpole temporalpole transversetemporal insula; do
      ...
      printf "%g," `grep -w ${x} ${subj_id}/stats/${side} | awk '{print $5}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_ThickAvg.csv
      printf "%g," `grep -w ${x} ${subj_id}/stats/${side} | awk '{print $3}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_SurfAvg.csv

    done
  done

  printf "%g," `cat ${subj_id}/stats/lh.aparc.stats | grep MeanThickness | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_ThickAvg.csv
  printf "%g," `cat ${subj_id}/stats/rh.aparc.stats | grep MeanThickness | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_ThickAvg.csv
  printf "%g," `cat ${subj_id}/stats/lh.aparc.stats | grep MeanThickness | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_SurfAvg.csv
  printf "%g," `cat ${subj_id}/stats/rh.aparc.stats | grep MeanThickness | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_SurfAvg.csv
  printf "%g," `cat ${subj_id}/stats/lh.aparc.stats | grep WhiteSurfArea | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_ThickAvg.csv
  printf "%g," `cat ${subj_id}/stats/rh.aparc.stats | grep WhiteSurfArea | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_ThickAvg.csv
  printf "%g," `cat ${subj_id}/stats/lh.aparc.stats | grep WhiteSurfArea | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_SurfAvg.csv
  printf "%g," `cat ${subj_id}/stats/rh.aparc.stats | grep WhiteSurfArea | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_SurfAvg.csv
  printf "%g" `cat ${subj_id}/stats/aseg.stats | grep IntraCranialVol | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_ThickAvg.csv
  printf "%g" `cat ${subj_id}/stats/aseg.stats | grep IntraCranialVol | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_SurfAvg.csv


  echo "" >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_ThickAvg.csv
  echo "" >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/CorticalMeasuresENIGMA_SurfAvg.csv

done


echo "SubjID, LLatVent,RLatVent,Lthal,Rthal,Lcaud,Rcaud,Lput,Rput,\
  Lpal,Rpal,Lhippo,Rhippo,Lamyg,Ramyg,Laccumb,Raccumb,ICV" > ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/LandRvolumes.csv
...
do #may need to change this so that is selects subjects with FS output
  ...
  do
    ...
    printf "%g," `grep  ${x} ${subj_id}/stats/aseg.stats | awk '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/LandRvolumes.csv

  done

printf "%g" `cat ${subj_id}/stats/aseg.stats | grep IntraCranialVol | awk -F, '{print $4}'` >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/LandRvolumes.csv

echo "" >> ${BASEDIR}/data/local/derivatives/freesurfer/7.4.1/ENIGMA_extract/LandRvolumes.csv
