...
Options:
    ...
    --cortex-only            Only map cortical parcels - do not map subcortical
...
Note: the subcortical operations assume that the reference is in MNI space. If this is not true (i.e. if you are creating T1w labels) use --cortex-only.
"""
...
class UserSettings:
    def __init__(self, arguments):
        ...
        self.cortex_only = arguments['--cortex-only']
...
def run_ciftify_dlabel_to_vol(arguments, tmpdir):
    ...
    if not settings.dlabel_in.has_volume or settings.cortex_only:

		#wb_command -volume-merge out.nii -volume first.nii
        ciftify.utils.run(['wb_command','-volume-merge',
            os.path.join(tmpdir, 'merge4d.nii.gz'),
			'-volume' , os.path.join(tmpdir, 'L.nii.gz'),
			'-volume', os.path.join(tmpdir, 'R.nii.gz')])
        ...
        # import the labels into subcort so merge works
        ciftify.utils.run(['wb_command', '-volume-label-import',
            subcort_final_labels,
            os.path.join(tmpdir, 'label_table.txt'),
            subcort_final_labels])
        ...
        ciftify.utils.run(['wb_command','-volume-merge',
            ...
            '-volume' , os.path.join(tmpdir, 'L.nii.gz'),
            '-volume', os.path.join(tmpdir, 'R.nii.gz'),
            '-volume', subcort_final_labels])

	# reduce the three using MAX (removing any overlaps)
    ciftify.utils.run(['wb_command', '-volume-reduce',
        os.path.join(tmpdir, 'merge4d.nii.gz'),
        'MAX',
